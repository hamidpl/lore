...
- `docusaurus.config.ts`: title/tagline/slug/org/copyright + `{{SITE_URL}}` + `{{LORE_ATTRIBUTION}}` + i18n (`{{LOCALE}}`, `{{LANG_LABEL}}`, `{{DIRECTION}}`, `{{HTML_LANG}}`). Pull product name/description from `.claude/CLAUDE.md` (tagline from the **Product Overview** block). **RTL:** set `customCss` to `['./src/css/custom.css', './src/css/custom-rtl.css']`; **LTR:** keep `['./src/css/custom.css']`.
  - `{{SITE_URL}}` is where **this documentation site** will be deployed (not the product's own website — those are different, and `/lore:config`'s "project site URL" is the product one). **Ask the user.** If they do not know yet, write `http://localhost:3000` and tell them to update `url` before their first deploy. Never leave a plausible-looking placeholder domain: `url` is the origin of every canonical link and every `sitemap.xml` entry, so a wrong-but-real-looking value publishes a sitemap for a domain nobody owns, and nothing in the build complains.
