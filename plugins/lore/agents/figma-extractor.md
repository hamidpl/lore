...
2. **Annotations:** fetch the node tree (`GET /v1/files/{key}/nodes?ids={id}`) and extract ALL `type: "TEXT"` nodes — these carry business rules. (Comments ≠ annotations; both are required — see lesson #5.)
3. **Frame inventory:** list frames with IDs/names; flag any page whose name starts with `[ignore]` as out of scope.
4. **Image export:** export **individual FRAME** nodes only (never SECTION — see lesson #1) at `scale=2`, batching ~5 at a time. Store under `static/img/{section}/`, referenced as `/img/{section}/` (never `/static/img/`).
