...
4. Run these source- and structure-specific checks (read-only):
   ...
   - **Scenario numbering + responsive-section check:** confirm each `###` scenario heading is numbered per the template (`Scenario N: …`); and if a doc references images under `/img/{section}/mobile/` or `/tablet/`, confirm a **Mobile & Tablet View** section exists and is not left as an empty `_(Optional …)_` stub. Apply exactly as the `lore:doc-reviewer` skill defines these checks — do not restate the rule.
