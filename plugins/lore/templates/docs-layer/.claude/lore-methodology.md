...
- ✅ **Write it before the first source payload is read.** A contract drafted after reading is anchored on what was found, not on what was asked — the instruction that conflicts with the material is exactly the one that then goes missing.
...
A language skill's own automation is advisory, not authoritative: run its checker, but read each finding before applying it, and never run a bulk auto-fixer over the tree. Such a tool normalises *text*, while a documentation file is text **and** structure — so the damage to expect is a prose rule applied where prose rules do not hold: inside a fenced code block, a table, a URL, a quoted UI string. Verified in one such tool at the time of writing: a whitespace normaliser with no fenced-code awareness, which flattens indentation inside a code block. Do not read that as an inventory — the point is the shape, not the list, and a fixed tool acquires new ones.
...
- ✅ **Fix in batches, and never edit a file while it is under review.** Collect every finding, work out what caused each, apply them together, then run **one** scoped round. Interleaving fixes with rounds is what turns one delivery into seven: in the run that produced these rules, four of seven rounds found defects that had not existed the round before, and the two rounds that finally came back clean were the two where the fixes went in as a batch and nothing was edited mid-round. **The batch is applied by `lore:doc-reviser`, never by the agent that wrote the page** — the actor that produced a claim is the worst-placed to judge which part of it to change, and a narrower instruction makes its collateral edits *worse*, not better.
- ✅ **Findings are routed by class, not all fixed alike.** The validator labels every finding `mechanical`, `content`, `evidence` or `decision` (the fields are the `lore:doc-reviewer` skill's). `mechanical` and `content` go to the reviser as one batch; `evidence` — a receipt or census row is missing — is unfinished extraction and goes back to it, never to a fixer that cannot fetch; `decision` goes to the user. A reviser that changes nothing does not earn a new round, and a fix the reviser rejects against the evidence is a disagreement the user settles, never something re-tried.
- ⛔ **Circuit breaker: two consecutive rounds that find defects introduced by the previous round means stop — and two reviser rounds since the last green verdict is the same stop.** Do not open a third. Report the pattern to the user — with the validator and reviser round counts from the run history the tooling keeps — and get a human decision about how to proceed. A self-feeding loop does not converge by being run again.

`lore:doc-validator` is read-only and never edits anything; `lore:doc-reviser` applies the validator's report as one batch and edits nothing the report does not name; the main agent invokes both, routes the findings, never edits a file under review, and stops after two reviser rounds.
...
- **`lore:doc-reviser` subagent** — a narrow-contract fixer that applies the validator's `mechanical`/`content` findings as one batch, at the targets the report names and nowhere else. It has no fetch, no file creation and no shell, so the one defect it cannot introduce is a fabricated receipt; a `SubagentStop` hook records each of its rounds in the same run history the validator uses.
