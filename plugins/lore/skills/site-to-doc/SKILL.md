...
Run scenarios **sequentially through one subagent invocation (or one per pass)** — never several `lore:site-explorer` workers at once: they would all share this session's single Playwright browser, fighting over tabs, viewport and auth state. Parallel extraction is a Figma-only pattern (see `lore:figma-to-doc`).
