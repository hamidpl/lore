...
| **§7 Language & Style** | Content language matches §7; English file/dir names; product-focused (non-marketing) tone. **Where the documentation language is right-to-left**, also run the two mechanical checks §7 names: an ordered-list marker in localised digits (`۱.`) where ASCII is required, and a missing `U+200F` RLM before a token that opens with a neutral character (a `/`, or a code span whose content starts with one). Both are **warnings**, not blocking: report the line and cite §7 for why, rather than restating it. An RLM before a strong-LTR letter is inert, so its absence is not a defect. Whether the prose itself reads natively is **not** yours to judge — §7 defers that to the language's own writing skill | methodology §7 |
