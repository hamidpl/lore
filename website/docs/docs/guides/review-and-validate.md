...
The reviewer itself never edits anything — it is read-only, and always was. The fixing used to fall to the main agent, and that is precisely where the loop came from: the actor that produced a claim is the worst-placed to judge which part of it to change. So the batch now goes to [`lore:doc-reviser`](../reference/skills-and-subagents.md), which edits only what the findings name and cannot fetch or create files — and while a verdict is blocking, a hook denies an edit made any other way. Findings that are not text fixes never reach it: a missing receipt goes back to source collection, and a product decision comes to you.
