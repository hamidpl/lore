...
> **The loop is not the validator's. It is the fixing.** `lore:doc-validator` is read-only and changes nothing; every fix is made by the main agent, under time pressure, one at a time, sometimes while a round is still in flight — and a fix is a claim that has been through none of the checks the original went through. Three of those seven rounds were self-inflicted this way.
