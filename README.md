...
| Command | `/lore:config` | Fill in / edit settings anytime (sources, roles, template, brand, language) |
...
- **Language drives styling.** An RTL language (Persian, Arabic, …) gets bundled RTL/Persian fonts + right-to-left layout; an LTR language (English, …) gets stock Docusaurus styling. The generated `CLAUDE.md` is language-agnostic — no Persian is hard-coded.
- **Nothing is locked in.** Run **`/lore:config`** anytime to fill or edit the product description, trusted sources (§1), user roles (§3), the document-writing template, the brand color — and even the 3 init answers (name / language / Docusaurus on-off). All `/lore:config` questions are skippable.
...
Then build content with `lore:figma-to-doc` / `lore:brief-to-doc` / `lore:site-to-doc`; validate with `lore:documentation-reviewer`; deploy (e.g. Cloudflare Pages: build `npm run build`, output `build`).

> A standalone `docusaurus-fa-starter` GitHub template also exists for the no-Claude-Code "Use this template" web flow; the `/lore:init` wizard is the recommended path when you have Claude Code.
...
- **Template layers** live under `templates/`: `docs-layer/` (always), `docusaurus-base/` (optional viewer), `rtl-assets/` (RTL languages only). The `docs-layer/CLAUDE.md` is the canonical thin product CLAUDE.md and uses `{{...}}` placeholders (including `{{DOC_LANGUAGE}}`/`{{LOCALE}}`) the wizard fills.
